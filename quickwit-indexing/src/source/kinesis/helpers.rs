...
pub(crate) mod tests {
    ...
    pub async fn put_records_into_shards<'a, K, I>(
        kinesis_client: &K,
        stream_name: &str,
        records: I,
    ...
    pub async fn teardown<K: Kinesis>(kinesis_client: &K, stream_name: &str) {
        let _delete_res = delete_stream(kinesis_client, stream_name).await;
    }

    pub async fn wait_for_active_stream<K: Kinesis>(
        kinesis_client: &K,
        stream_name: &str,
    ) -> anyhow::Result<()> {
